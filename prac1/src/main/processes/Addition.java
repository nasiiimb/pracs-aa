...
/**
 * Simulates the complexity of the addition of two matrices, O(n^2).
 */
public class Addition extends Thread implements Notify {
    ...
    public Addition(Controller controller, Data data) {
    ...
    public void run() {
        ...
        for (int index = 0; (index < data.getElementsSize()) && (!cancel); index++) {
            ...
            Matrix result = Matrix.add(a, b, controller);

            if (!cancel) {
                long endTime = System.nanoTime();
                long duration = endTime - startTime;
                data.setAdditionTime(duration);

                if (k == 0 && index == 0) {
                    // Calculate the constant k
                    this.k = duration / (double) (n * n);
                    System.out.println("------------------------------");
                    System.out.printf("N = %d, Constant Multiplicativa de les sumes de matrius = %.4f\n", n, this.k);
                } else {
                    // Calculate the estimated execution time
                    double estimatedTime = this.k * (n * n);
